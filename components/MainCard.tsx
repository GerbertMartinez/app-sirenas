...
const MainCard = ({
    ...
    const {onAll, offAll, testAll} = useActions();
    ...
    const onTest = () => {
        ...
        setType('T');
        setTitle('Atención');
        setMessage(`¿Estás seguro de realizar el test en ${name}?`);
    ...
    const onOn = () => {
        ...
        setType('O');
        setTitle('Atención');
        setMessage(`¿Estás seguro de encender ${name}?`);
    ...
    return (
        <PulseView trigger={sounding} primary={colors.rmuni[100]} secondary={colors.charcoal}>
            <Pressable 
                className="bg-steel p-6 rounded-3xl" 
                onPress={() => router.push('/mainDetail')}
                disabled={loading}
            >
                ...
                {loading ? (
                    <View className="flex-row justify-between mb-5">
                        <View className="flex-row items-center bg-silver/20 border border-silver/40 px-3 py-2 rounded-xl">
                ...
                <View className="flex-row justify-between gap-3">
                    ...
                    <Pressable
                        onPress={onTest}
                        className="flex-1 py-3 rounded-2xl justify-center items-center border border-acmuni-100 bg-acmuni-100/20 active:opacity-60"
                        disabled={loading}
                    >
                        {loading ? (
                            <ActivityIndicator color={colors.acmuni[100]} />
                        ):(
                    ...
                    <Pressable
                        onPress={onOff}
                        className="flex-1 py-3 rounded-2xl justify-center items-center border border-silver bg-silver/20 active:opacity-60"
                        disabled={loading}
                    >
                        {loading ? (
                            <ActivityIndicator color={colors.silver} />
                    ...
                    <Pressable
                        onPress={onOn}
                        className="flex-1 py-3 rounded-2xl justify-center items-center border border-rmuni-100 bg-rmuni-100/20 active:opacity-60"
                        disabled={loading}
                    >
                        {loading ? (
                            <ActivityIndicator color={colors.rmuni[100]} />
                        ):(
                            <View className="flex-row">
